...
class InstallationThread(ProcessThread):
    ...
    def run(self):
        try:
            ...
            self.log_signal.emit(
                f'[DEBUG] Unpacking AE from {
                    self.ae_filename}...')
            self.unpack_zip(self.ae_filename,
                            get_aegnux_installation_dir().as_posix())
            os.rename(get_aegnux_installation_dir().joinpath(
                'Support Files'), get_ae_install_dir())

            self.progress_signal.emit(20)

            self.log_signal.emit(
                f'[DEBUG] Copying Wine Runner from {WINE_RUNNER_DIR}...')
            shutil.copytree(
                WINE_RUNNER_DIR,
                get_wine_runner_dir(),
                dirs_exist_ok=True)
            ...
            self.log_signal.emit(
                f'[DEBUG] Initializing wineprefix in {
                    get_wineprefix_dir()}...')
            ...
            self.run_command(
                ['wine', 'regedit', WINE_STYLE_REG], in_prefix=True)
            ...
            for tweak in tweaks:
                self.log_signal.emit(
                    f'[DEBUG] Installing {tweak} with winetricks')
            ...
            self.run_command(['wine', get_vcr_dir_path().joinpath(
                'install_all.bat').as_posix()], in_prefix=True)

            self.progress_signal.emit(85)

            self.log_signal.emit(
                f'[DEBUG] Unpacking MSXML3 to {
                    get_msxml_dir_path()}...')
            ...
            shutil.copy(
                get_msxml_dir_path().joinpath('msxml3.dll'),
                system32_dir.joinpath('msxml3.dll'))
            shutil.copy(
                get_msxml_dir_path().joinpath('msxml3r.dll'),
                system32_dir.joinpath('msxml3r.dll'))
            ...
            if is_nvidia_present():
                self.log_signal.emit(
                    "[INFO] Starting NVIDIA libs installation...")
    ...
    def install_nvidia_libs(self):
        ...
        returncode = self.run_command(
            [setup_script.as_posix(), 'install'], in_prefix=True)

        if returncode != 0:
            self.log_signal.emit(
                f"[ERROR] Setup script failed with return code {returncode}.")
            self.finished_signal.emit(False)
            return
